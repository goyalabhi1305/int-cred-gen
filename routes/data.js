...
const StudentsProfile = require('../models/data')
...
router.get('/create', async (req, res) => {
    try {
        ...
        const insert = await new StudentsProfile({
            name: 'Harry Potter and the Order of the Phoenix',
            img: 'https://bit.ly/2IcnSwz',
            string: `Harry Potter and Dumbledore's warning about the return of
            Lord Voldemort is not heeded by the wizard authorities who, in turn, look to
            undermine Dumbledore's authority at Hogwarts and discredit Harry.`
        }).save()
        if (insert) {
...
router.get('/read', async (req, res) => {
    try {
        const data = await StudentsProfile.find()
        if (data) {
...
router.get('/update', async (req, res) => {
    try {
        const data = await StudentsProfile.updateOne({name:'Harry Potter and the Order of the Phoenix'},{$set:{name:'Abhi'}})
        if (data) {
            res.status(200).json({msg:'success' })
            }
            else {
...
router.get('/delete', async (req, res) => {
    try {
        const data = await StudentsProfile.deleteOne({name:'Harry Potter and the Order of the Phoenix'})
        if (data) {
            res.status(200).json({msg:'success' })
            }
            else {
